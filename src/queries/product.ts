...
export const getProducts = async (
  ...
  if (filters.isSale !== undefined) {
    if (filters.isSale === "yes") {
      wherClause.AND.push({
        variants: {
          some: {
            isSale: true,
            OR: [
              { saleEndDate: null }, // If sale has no end date
              { saleEndDate: { gte: new Date().toISOString() } }, // Sale is still active
            ],
          },
        },
      });
    } else if (filters.isSale === "no") {
      wherClause.AND.push({
        NOT: {
          variants: {
            some: {
              isSale: true,
              OR: [
                { saleEndDate: null }, // Exclude ongoing sales
                { saleEndDate: { gte: new Date().toISOString() } }, // Exclude active sales
              ],
            },
          },
        },
