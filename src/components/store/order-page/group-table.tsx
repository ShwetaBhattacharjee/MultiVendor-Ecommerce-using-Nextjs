...
import { useCurrency } from "../home/CurrencyContext"; // Import currency context
import { convertCurrency } from "@/lib/currencyConverter"; // Import conversion function
...
export default function OrderGroupTable({
  ...
  const { selectedCurrency, exchangeRates } = useCurrency(); // Get currency and rates
  ...
  const convertedSubTotal = convertCurrency(
    subTotal,
    exchangeRates,
    selectedCurrency
  );
  const convertedShippingFees = convertCurrency(
    shippingFees,
    exchangeRates,
    selectedCurrency
  );
  const convertedDiscountedAmount = convertCurrency(
    discountedAmount,
    exchangeRates,
    selectedCurrency
  );
        ...
        >
          ...
          <p className="font-medium text-lg text-gray-900 px-6 py-3 max-lg:text-center">
            ...
            <span className="text-gray-500 ms-1">
              {convertedSubTotal} {selectedCurrency}
            </span>
          </p>
          ...
          <p className="font-medium text-lg text-gray-900 px-6 py-3 max-lg:text-center">
            Shipping Fees:
            <span className="text-gray-500 ms-1">
              {convertedShippingFees} {selectedCurrency}
            </span>
          </p>
          ...
          {group.couponId && (
            <p className="font-medium text-lg text-gray-900 pl-6 py-3 max-lg:text-center">
              ...
              <span className="text-gray-500 ms-1">
                (-{convertedDiscountedAmount} {selectedCurrency})
              </span>
            </p>
          )}
        </div>
