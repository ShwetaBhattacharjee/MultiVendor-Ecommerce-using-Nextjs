...
import { useCurrency } from "../home/CurrencyContext"; // Import the currency hook
import { convertCurrency } from "@/lib/currencyConverter"; // Import the currency conversion utility
...
const CartProduct: FC<Props> = ({
  ...
  // Get currency and exchange rates from the context
  const { selectedCurrency, exchangeRates } = useCurrency();
  const baseCurrency = "USD"; // Default base currency
  const conversionRate = exchangeRates[selectedCurrency] || 1; // Get selected currency rate
  const baseRate = exchangeRates[baseCurrency] || 1; // Get base currency rate
              ...
              {stock > 0 ? (
                <div>
                  <span className="inline-block break-all">
                    {selectedCurrency} {convertPrice(price).toFixed(2)} x{" "}
