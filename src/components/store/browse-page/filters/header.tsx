    ...
    <div className="pt-2.5 pb-5">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {queriesArray.map(([queryKey, queryValue]) => {
          ...
          // ✅ Fix: Filter out non-string values before passing them
          const sanitizedQueryValues = queryValues.filter(
            (item): item is string => typeof item === "string"
          );

          return (
            <div key={queryKey} className="flex flex-wrap gap-2">
              {sanitizedQueryValues.map((value, index) => (
                <div
                  key={index}
                  ...
                  <X
                    ...
                    onClick={() => {
                      isArrayQuery
                        ? handleRemoveQuery(
                            queryKey,
                            sanitizedQueryValues,
                            value
                          ) // ✅ Only pass sanitized values
